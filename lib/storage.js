...
export class StorageArea {
  constructor(areaName, config) {
    ...
    if (areaName === "sync") {
      this.config = config;
      if (config && typeof config.interval === "number") {
  ...
  set(items, callback) {
    ...
    function createOrUpdateItem(item) {
      ...
      function createItem() {
        onChanged._addChange(areaName, id, null, record);
        return this_items.create(record, {useRecordId: true});
      }

      function updateItem(old_record) {
        onChanged._addChange(areaName, id, old_record, record);
...
export const onChanged = new Events();
export const sync = new StorageArea("sync", {
  type: "kinto",
  interval: 10000,
  remote: "https://kinto.dev.mozaws.net/v1/",
  headers: {Authorization: "Basic " + btoa("user:pass")}
});
export const local = new StorageArea("local");
export const managed = new StorageArea("managed");
